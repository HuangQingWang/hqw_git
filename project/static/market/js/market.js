$(document).ready(function(){
    ...
    $.post("/bridgr/",{"tempdata":temp},function (data) {
        var ys = document.getElementsByClassName("yellowSlide")
        for(var i=0;i<ys.length;i++){
            var a = ys[i].getAttribute("yellowflage")
            if(data.currentpageid == a){
                ys[i].style.background = "yellow"
            }
            else{
                ys[i].style.background = "#fafafa"
            }
        }
    })
